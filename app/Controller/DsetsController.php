...
/**
 * Class DsetsController
 */
class DsetsController extends AppController
{
	public $uses=['Dataset','Journal','Report','Quantity','Dataseries',
		'Parameter','Variable','Scidata','System','Substance','File','Reference','Unit','Sampleprop','Trc'];
	...
	public function view($id,$serid=null,$layout=null)
	{
		...
		//debug($cont);exit;
		$name= $dump["Dataseries"][0]["Datapoint"][0];
		$xname=$name["Condition"][0]["Property"]["name"];
		$xunit=$name["Condition"][0]["Unit"]["label"];
		$xlabel=$xname.", ".$xunit;
		$condition=$dump["Dataseries"];
		foreach($condition as $con){
			if(isset($con["Condition"][1])) {
				$cont=$con["Condition"][1]["number"]+0;
				$conditionunit=$con["Condition"][1]["Unit"]["symbol"];
			} else {
				$cont=$con["Condition"][0]["number"]+0;
				$conditionunit=$con["Condition"][0]["Unit"]["symbol"];
			}
			$cond=$cont." ".$conditionunit;
			$conds[]=$cond;
		}
		// debug($cond);exit;
		$yunit=$name["Data"][0]["Unit"]["header"];
		$samprop= $name["Data"][0]["Sampleprop"]=["property_name"];
		$ylabel=$samprop;
